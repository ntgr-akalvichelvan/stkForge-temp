/* 
 * This file is derived from a version contained in u-boot and was
 * modified by Broadcom Corp. for use in a standalone application 
 * outside of u-boot. 
 *  
 * All such changes are marked with BRCM_FIXUP and are summarized
 * in this change log: 
 *  
 * 2016-Jan-28 
	...
	if ((argc >= 3) && (strcmp(argv[2], "-") ==  0)) {
		...
		case IMAGE_FORMAT_FIT:
			...
			if (!fit_check_format(fit_hdr)) {
				printf("Bad FIT ramdisk image format!\n");
				return 1;
			}
			...
			if (fit_image_get_data(fit_hdr, rd_noffset, &data,
						&size)) {
				printf("Could not find ramdisk subimage data!\n");
				return 1;
			}
			...
			if (fit_image_get_load(fit_hdr, rd_noffset, &rd_load)) {
				printf("Can't get ramdisk subimage load "
					"address!\n");
				return 1;
			}
...
#endif
			{
				printf("Wrong Ramdisk Image Format\n");
	...
	if (rd_data) {
		if (!initrd_copy_to_ram) {	/* zero-copy ramdisk support */
			...
			*initrd_end = rd_data + rd_len;
			...
			if (*initrd_start == 0) {
				printf("ramdisk - allocation error\n");
				goto error;
			}
...
int boot_relocate_fdt(struct lmb *lmb, char **of_flat_tree, ulong *of_size)
{
		...
		if (((ulong) desired_addr) == ~0UL) {
			...
			of_start =
			    (void *)(ulong) lmb_alloc_base(lmb, of_len, 0x1000,
							   (ulong)desired_addr);
			if (of_start == 0) {
				printf("Failed using fdt_high value for Device Tree");
				goto error;
			}
		} else {
			of_start =
	...
	if (of_start == 0) {
		printf("device tree - allocation error\n");
		goto error;
	}
	...
	*of_size = 0;
	...
	if (argc > 3 || genimg_has_config(images)) {
		...
		case IMAGE_FORMAT_FIT:
			...
			break;
		default:
			printf("ERROR: Did not find a cmdline Flattened Device "
				"Tree\n");
			goto error;
		}
